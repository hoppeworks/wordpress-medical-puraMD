...
! defined( 'WOOSB_VERSION' ) && define( 'WOOSB_VERSION', '3.8.1' );
		...
		if ( ! class_exists( 'WC_Product_Woosb' ) && class_exists( 'WC_Product' ) ) {
			class WC_Product_Woosb extends WC_Product {
				...
				public function get_sale_price( $context = 'view' ) {
					if ( ! $this->is_fixed_price() ) {
						...
						if ( ( $discount > 0 ) || ( $discount_amount > 0 ) ) {
							...
							if ( $woosb_items = $this->get_items() ) {
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product->is_type( 'variable' ) ) {
										$sale_price += $woosb_product->get_variation_price( 'min' ) * $woosb_item['qty'];
									} else {
										$sale_price += $woosb_product->get_price() * $woosb_item['qty'];
									}
								}
							}
							...
							return round( $sale_price * ( 100 - $discount ) / 100, wc_get_price_decimals() );
						}
					}
				...
				public function get_discount() {
					...
					// discount percentage
					if ( ! $this->is_fixed_price() && ( $woosb_discount = get_post_meta( $product_id, 'woosb_discount', true ) ) && is_numeric( $woosb_discount ) && ( (float) $woosb_discount < 100 ) && ( (float) $woosb_discount > 0 ) ) {
			...
			class WPcleverWoosb {
				...
				function woosb_update_price_for_id( $product_id ) {
					...
					if ( $product && $product->is_type( 'woosb' ) && ! $product->is_fixed_price() ) {
						...
						$woosb_items = $this->woosb_get_bundled( $product_id );
				...
				function woosb_wp_enqueue_scripts() {
					...
					wp_localize_script( 'woosb-frontend', 'woosb_vars', array(
							...
							'wrap_selector'            => apply_filters( 'woosb_wrap_selector', '.product-type-woosb' ),
				...
				function woosb_row_meta( $links, $file ) {
					...
					if ( $plugin === $file ) {
						$row_meta = array(
							'support' => '<a href="https://wpclever.net/support" target="_blank">' . esc_html__( 'Premium support', 'woo-product-bundle' ) . '</a>',
						);
				...
				function woosb_get_item_count( $count, $item_type, $order ) {
					...
					if ( $cart_contents_count !== 'both' ) {
						...
						if ( ( $cart_contents_count === 'bundled_products' ) && ( $order_bundled > 0 ) ) {
							return $count - $order_bundles;
						}

						if ( ( $cart_contents_count === 'bundle' ) && ( $order_bundles > 0 ) ) {
				...
				function woosb_add_to_cart_validation( $passed, $product_id ) {
					if ( $woosb_ids = get_post_meta( $product_id, 'woosb_ids', true ) ) {
						if ( isset( $_POST['woosb_ids'] ) ) {
							$woosb_ids = $_POST['woosb_ids'];
						}

						$woosb_qty = isset( $_POST['quantity'] ) ? (int) $_POST['quantity'] : 1;
						$woosb_ids = $this->woosb_clean_ids( $woosb_ids );

						if ( $woosb_items = $this->woosb_get_bundled( 0, $woosb_ids ) ) {
							foreach ( $woosb_items as $woosb_item ) {
								$woosb_item_id  = $woosb_item['id'];
								$woosb_item_qty = $woosb_item['qty'];
								...
								if ( $woosb_product->is_type( 'variable' ) || $woosb_product->is_type( 'woosb' ) ) {
									wc_add_notice( sprintf( esc_html__( '"%s" is un-purchasable.', 'woo-product-bundle' ), esc_html( $woosb_product->get_name() ) ), 'error' );
									wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-product-bundle' ), 'error' );

									return false;
								}

								if ( ! $woosb_product->is_in_stock() || ! $woosb_product->is_purchasable() ) {
									wc_add_notice( sprintf( esc_html__( '"%s" is un-purchasable.', 'woo-product-bundle' ), esc_html( $woosb_product->get_name() ) ), 'error' );
									wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-product-bundle' ), 'error' );
				...
				function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
					...
					$woosb_discount        = WC()->cart->cart_contents[ $cart_item_key ]['data']->get_discount();
					$woosb_discount_amount = WC()->cart->cart_contents[ $cart_item_key ]['data']->get_discount_amount();

					WC()->cart->cart_contents[ $cart_item_key ]['woosb_fixed_price']     = $woosb_fixed_price;
					WC()->cart->cart_contents[ $cart_item_key ]['woosb_discount']        = $woosb_discount;
					WC()->cart->cart_contents[ $cart_item_key ]['woosb_discount_amount'] = $woosb_discount_amount;

					$exclude_unpurchasable = get_option( '_woosb_exclude_unpurchasable', 'no' );

					if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
						foreach ( $items as $item ) {
							$woosb_item_id  = $item['id'];
							$woosb_item_qty = $item['qty'];
							...
							if ( ! $woosb_fixed_price && ( $woosb_discount_amount <= 0 ) && ( $woosb_discount > 0 ) ) {
								$woosb_item_price *= (float) ( 100 - $woosb_discount ) / 100;
							}
							...
							$woosb_item_data   = array(
								...
								'woosb_discount'        => $woosb_discount,
								'woosb_discount_amount' => $woosb_discount_amount
							);
				...
				function woosb_before_calculate_totals( $cart_object ) {
					...
					foreach ( $cart_object->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( ! empty( $cart_item['woosb_parent_id'] ) ) {
							...
							} elseif ( isset( $cart_item['woosb_price'], $cart_item['woosb_discount'], $cart_item['woosb_discount_amount'] ) && ( $cart_item['woosb_discount_amount'] <= 0 ) && ( $cart_item['woosb_discount'] > 0 ) ) {
						...
						if ( ! empty( $cart_item['woosb_ids'] ) && ! empty( $cart_item['woosb_keys'] ) && isset( $cart_item['woosb_fixed_price'] ) && ! $cart_item['woosb_fixed_price'] ) {
							...
							foreach ( $cart_item['woosb_keys'] as $woosb_key ) {
								if ( isset( $cart_items[ $woosb_key ] ) ) {
									$bundle_price += round( $cart_items[ $woosb_key ]['line_subtotal'], wc_get_price_decimals() );

									if ( $cart_object->round_at_subtotal ) {
										// round tax at subtotal
										$bundle_tax += $cart_items[ $woosb_key ]['line_subtotal_tax'];
									} else {
										$bundle_tax += round( $cart_items[ $woosb_key ]['line_subtotal_tax'], wc_get_price_decimals() );
									}
								}
							}

							if ( $cart_object->tax_display_cart === 'incl' ) {
								$bundle_price += round( $bundle_tax, wc_get_price_decimals() );
							}

							if ( ! empty( $cart_item['woosb_discount_amount'] ) ) {
								$bundle_price -= $cart_item['woosb_discount_amount'] * $cart_item['quantity'];
							}

							WC()->cart->cart_contents[ $cart_item_key ]['woosb_price'] = round( $bundle_price / $cart_item['quantity'], wc_get_price_decimals() );
						}
					}
				}
				...
				function woosb_get_item_data( $item_data, $cart_item ) {
					...
					$woosb_items     = $this->woosb_get_bundled( 0, $cart_item['woosb_ids'] );
					$woosb_items_str = '';

					if ( is_array( $woosb_items ) && count( $woosb_items ) > 0 ) {
						foreach ( $woosb_items as $woosb_item ) {
							$woosb_items_str .= $woosb_item['qty'] . ' × ' . get_the_title( $woosb_item['id'] ) . '; ';
						}
					}
				...
				function woosb_checkout_create_order_line_item( $item, $cart_item_key, $values, $order ) {
					...
					$woosb_items     = $this->woosb_get_bundled( 0, $values['woosb_ids'] );
					$woosb_items_str = '';
				...
				function woosb_ajax_add_order_item_meta( $item_id, $item, $order ) {
					...
					if ( 'line_item' === $item->get_type() ) {
						...
						if ( $product && $product->is_type( 'woosb' ) && ( $woosb_items = $product->get_items() ) ) {
							// get bundle info
							$woosb_fixed_price     = $product->is_fixed_price();
							$woosb_discount        = $product->get_discount();
							$woosb_discount_amount = $product->get_discount_amount();

							// add the bundle
							if ( ! $product->is_fixed_price() ) {
								if ( $woosb_discount_amount > 0 ) {
									$product->set_price( - $woosb_discount_amount );
								} else {
									$product->set_price( 0 );
								}
							}

							$woosb_order_id = $order->add_product( $product, $quantity );
							...
							foreach ( $woosb_items as $woosb_item ) {
								...
								if ( $woosb_fixed_price ) {
									$woosb_item_product->set_price( 0 );
								} elseif ( $woosb_discount > 0 ) {
									$woosb_item_price = (float) ( 100 - $woosb_discount ) * $woosb_item_product->get_price() / 100;
									$woosb_item_product->set_price( $woosb_item_price );
								}

								// add bundled products
								$woosb_order_item_id = $order->add_product( $woosb_item_product, $woosb_item['qty'] * $quantity );

								if ( ! $woosb_order_item_id ) {
									continue;
								}

								$woosb_order_items = $order->get_items( 'line_item' );
								$woosb_order_item  = $woosb_order_items[ $woosb_order_item_id ];
								$woosb_order_item->add_meta_data( '_woosb_parent_id', $product_id, true );
								$woosb_order_item->save();
							}
				...
				function woosb_product_data_panels() {
					...
					if ( get_post_meta( $post_id, 'woosb_ids', true ) ) {
						$woosb_ids_str = get_post_meta( $post_id, 'woosb_ids', true );
					} elseif ( isset( $_GET['woosb_ids'] ) ) {
						$woosb_ids_str = implode( ',', explode( '.', $_GET['woosb_ids'] ) );
					}

					$woosb_ids_str = $this->woosb_clean_ids( $woosb_ids_str );
                    ...
                    <div id='woosb_settings' class='panel woocommerce_options_panel woosb_table'>
                        <table>
                                ...
                                <td>
                                    <div class="w100">
                                        <input type="hidden" id="woosb_ids" class="woosb_ids" name="woosb_ids"
                                               value="<?php echo esc_attr( $woosb_ids_str ); ?>"
                                               readonly/>
                                        <div id="woosb_selected" class="woosb_selected">
                                            <ul>
												<?php
												if ( ! empty( $woosb_ids_str ) ) {
													$woosb_items = $this->woosb_get_bundled( 0, $woosb_ids_str, false );
													if ( is_array( $woosb_items ) && count( $woosb_items ) > 0 ) {
														foreach ( $woosb_items as $woosb_item ) {
															$woosb_product = wc_get_product( $woosb_item['id'] );
															if ( ! $woosb_product || $woosb_product->is_type( 'woosb' ) ) {
															...
															$this->woosb_product_data_li( $woosb_product, $woosb_item['qty'] );
														}
													}
												}
												?>
                            ...
                            <tr class="woosb_tr_space">
                                <th><?php esc_html_e( 'Optional', 'woo-product-bundle' ); ?></th>
                                <td>
						...
						if ( ! $product->is_fixed_price() ) {
							...
							if ( $product->is_optional() ) {
								...
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product ) {
										if ( $woosb_product->is_type( 'variable' ) ) {
											$price += wc_get_price_to_display( $woosb_product, array(
												'qty'   => $woosb_item['qty'],
												'price' => $woosb_product->get_variation_price( 'min' )
											) );
										} else {
											$price += wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) );
										}
									}
								}
								...
								if ( $discount_amount > 0 ) {
									...
									$price_sale = $price * (float) ( 100 - $discount ) / 100;
								}
				...
				function woosb_order_again_cart_item_data( $item_data, $item ) {
					if ( isset( $item['woosb_ids'] ) ) {
						...
						$item_data['woosb_ids']         = $item['woosb_ids'];
					}

					if ( isset( $item['woosb_parent_id'] ) ) {
						$item_data['woosb_order_again'] = 'yes';
						$item_data['woosb_parent_id']   = $item['woosb_parent_id'];
					}
				...
				function woosb_show_bundled( $product = null ) {
					...
					if ( $woosb_items = $product->get_items() ) {
						...
						echo '<div class="woosb-wrap woosb-bundled">';
                        ...
                        <div class="woosb-products"
							...
							<?php foreach ( $woosb_items as $woosb_item ) {
								...
								if ( ! $woosb_product || ( $product_count > 2 ) ) {
									continue;
								}

								$woosb_product_qty     = $woosb_item['qty'];
								$woosb_product_qty_min = 0;
								$woosb_product_qty_max = 1000;
                                ...
                                <div class="woosb-product"
                                    ...
                                    <div class="woosb-title">
										...
										if ( $woosb_product->is_type( 'variable' ) ) {
											if ( ( get_option( '_woosb_variations_selector', 'default' ) === 'wpc_radio' ) && class_exists( 'WPclever_Woovr' ) ) {
												WPclever_Woovr::woovr_variations_form( $woosb_product );
											} else {
												...
												if ( is_array( $attributes ) && ( count( $attributes ) > 0 ) ) {
													echo '<div class="variations_form" data-product_id="' . absint( $woosb_product->get_id() ) . '" data-product_variations="' . $variations_attr . '">';
									...
									<?php if ( get_post_meta( $product_id, 'woosb_optional_products', true ) === 'on' ) {
										...
										if ( $woosb_product->is_in_stock() ) {
                                            ...
                                            <div class="woosb-qty">
												<?php
												if ( apply_filters( 'woosb_show_minus', false ) ) {
													echo '<span class="woosb-minus">-</span>';
												}
												woocommerce_quantity_input( array(
													'input_value' => $woosb_product_qty,
													'min_value'   => $woosb_product_qty_min,
													'max_value'   => $woosb_product_qty_max
												), $woosb_product );
												if ( apply_filters( 'woosb_show_plus', false ) ) {
													echo '<span class="woosb-plus">+</span>';
												}
												?>
                                            </div>
											<?php
									...
									<?php if ( get_option( '_woosb_bundled_price', 'price' ) !== 'no' ) { ?>
                                        <div class="woosb-price">
                                            <div class="woosb-price-ori">
												<?php
												if ( ! $product->is_fixed_price() && ! $product->get_discount_amount() && ( ( $discount = $product->get_discount() ) > 0 ) ) {
													$show_new_price      = true;
													$woosb_product_price = $woosb_product->get_price() * ( 100 - $discount ) / 100;
												} else {
													$show_new_price      = false;
													$woosb_product_price = $woosb_product->get_price();
												}

												switch ( get_option( '_woosb_bundled_price', 'price' ) ) {
													case 'price':
														if ( $show_new_price ) {
															$woosb_price = wc_format_sale_price( wc_get_price_to_display( $woosb_product ), wc_get_price_to_display( $woosb_product, array( 'price' => $woosb_product_price ) ) );
														} else {
													...
													case 'subtotal':
														if ( $show_new_price ) {
															$woosb_price = wc_format_sale_price( wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) ), wc_get_price_to_display( $woosb_product, array(
																'price' => $woosb_product_price,
																'qty'   => $woosb_item['qty']
															) ) );
														} else {
				...
				function woosb_get_bundled( $product_id, $ids = null, $compact = true ) {
					$woosb_arr = array();
					$woosb_ids = ! is_null( $ids ) ? $ids : get_post_meta( $product_id, 'woosb_ids', true );
					...
					if ( ! empty( $woosb_ids ) ) {
						...
						if ( is_array( $woosb_items ) && count( $woosb_items ) > 0 ) {
							foreach ( $woosb_items as $woosb_item ) {
								...
								$has_id = false;
								if ( $compact && ( count( $woosb_arr ) > 0 ) ) {
									foreach ( $woosb_arr as $woosb_k => $woosb_it ) {
										if ( $woosb_it['id'] === $woosb_item_id ) {
											$woosb_arr[ $woosb_k ]['qty'] += $woosb_item_qty;
											$has_id                       = true;
											break;
										}
									}
				...
				function woosb_display_post_states( $states, $post ) {
					if ( 'product' == get_post_type( $post->ID ) ) {
						if ( ( $product = wc_get_product( $post->ID ) ) && $product->is_type( 'woosb' ) ) {
							...
							if ( $woosb_items = $product->get_items() ) {
								$woosb_count = count( $woosb_items );
							}

							$states[] = apply_filters( 'woosb_post_states', sprintf( esc_html__( 'Bundle (%s)', 'woo-product-bundle' ), $woosb_count ), $woosb_count, $product );
						}
					}
				...
				function woosb_bulk_action_handler( $redirect_to, $doaction, $post_ids ) {
					...
					$woosb_ids   = implode( '.', $post_ids );
					$redirect_to = add_query_arg( 'woosb_ids', $woosb_ids, admin_url( 'post-new.php?post_type=product' ) );
				...
				function woosb_bulk_action_admin_notice() {
					if ( ! empty( $_REQUEST['woosb_ids'] ) ) {
						$woosb_ids = explode( '.', $_REQUEST['woosb_ids'] );
						echo '<div id="message" class="updated fade">' . sprintf( esc_html__( 'Added %s product(s) to this bundle.', 'woo-product-bundle' ), count( $woosb_ids ) ) . '</div>';
					}
				}
